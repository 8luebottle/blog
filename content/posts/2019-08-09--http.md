...
## HTTP Request Structure
  ...
  Headers의 종류로는 General Header, Request Header, Response Header, Entity Header가 있다.<br>
...
### Connectionless
  ...
  대부분의 경우 적은수의 서버가 많은 클라이언트의 요청을 받아들인다. 만약 서버가 수백개 또는 그 이상의 클라이언트와의 연결을 끊지않고 유지한다면 

### Stateless
  HTTP는 stateless 프로토콜이다. State 는 '상태'를 의미한다. 상태가 없다는 것은 요청과 응답의 상태를 저장하지 않는다는 것을 말한다.
...
# HTTP Methods
  ...
  주로 진단을 위해 사용되는 TRACE 메서드는 클라이언트에게 자신의 요청이 서버에 도착했을 때에 어떻게 보이기 되는지 알려준다. Client가 보낸 TRACE 요청은 Server에서 loopback 진단을 한다. 요청 전송의 마지막 단계에서 Server는 자신이 받은 요청 메세지를 본문에 넣어 TRACE 응답을 되돌려준다.
...
# HTTP Status Code
  ...
  - 205 Reset Content<br>
    <small>브라우저를 위한 상태 코드이다. 브라우저에게 현재 페이지에 있는 HTML 폼에 있는 모든 값을 비우도록 하기 위해 사용.</small>
  ...
  리다이렉션 상태코드는 클라이언트가 관심있어 하는 리소스에 대해 다른위치에 관한 정보를 알려주거나 그 리소스의 내용 대신 다른 대안을 알려준다.
  ...
  - 500 Internal Server Error<br>
    <small>내부 서버 오류. 서버가 요청을 처리할 수 없는 에러를 민닜을때 사용.</small>
  ...
  - 504 Gateway Timeout<br>
    <small>게이트웨이 시간 초과. 다른 서버에게 요청을 보내고 응답을 기다리다 타임아읏이 발생한 Gateway나 Proxy에서 온 응답을 말한다.</small>
