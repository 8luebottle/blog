...
개발자가 갖추어야할 전문지식을 얻을 수 있는 방법은 학교, 독학, 부트캠프, 국비지원 등이 있다. 이 선택지 가운데 나는 부트캠프중 하나인 코드스테이츠로 결정을 내렸다. 페어프로그래밍, 깃(git) 사용, 커뮤니티는 내가 코드스테이트를 결정하게 된 주요 이유였다. 코드스테이츠의 정규과정인 이머시브(Immersive) 클래스는 총 3개월 동안 진행된다. 프로그래밍에 제로베이스를 가진 나로써는 이머시브를 수강하기에 앞서 2개월 과정인 프리 코스를 진행해야 했다. 코드스테이트의 커리큘럼은 주입식교육법을 따르지 않는다. 대신에 각자에 맞는 학습방법을 발전시키고 문제해결 역량을 키울 수 있도록 중점을 두었다. 이는 일생동안 끊임 없이 전문지식을 학습해나가야 하는 개발자에게 필요한 역량이다. 
